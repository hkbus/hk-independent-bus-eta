  ...
  en: {
    translation: {
      ...
      "讓多部裝置共用同一組收藏（車站、到站預報、集合）及個人化設定。掃描QR碼即可加入。":
        "Share the same favorites (stops, ETAs, collections) and personal settings across multiple devices. Scan a QR code to join.",
      立即同步: "Sync now",
      顯示QR碼以加入其他裝置: "Show QR code to add another device",
      "離開後，此裝置的收藏及設定將不再與其他裝置同步，但不會刪除任何裝置上的資料。":
        "After leaving, this device's favorites and settings will stop syncing with other devices, but no data will be deleted from any device.",
      確定離開: "Leave",
      不離開: "Don't leave",
      ...
      "在另一部裝置的「同步群組」中顯示QR碼，然後用相機應用程式掃描；或直接貼上連結/代碼。":
        "Open “Sync Group” on another device to show its QR code, then scan it with your camera app; or paste the link/code directly.",
      連結或代碼: "Link or code",
      加入: "Join",
