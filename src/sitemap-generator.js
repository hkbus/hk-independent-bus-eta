...
request('https://hkbus.github.io/hk-bus-crawling/routeFareList.min.json', (e, r, b) => {
  ...
  zhRoutes = Object.entries(routeList).map(route => `/zh/route/${route[0]}`)
  enRoutes = Object.entries(routeList).map(route => `/en/route/${route[0]}`)
  
  // write to file
  fs.writeFileSync('sitemap.txt', host+"\n"+[].concat(zhPages, enPages, zhRoutes, enRoutes).map(p => `${host}${p.replace(/ /g, '%20')}`).join("\n"))
