...
const RouteSearch = () => {
  ...
  const updateRoutes = (routeResults) => {
    ...
    Promise.all(
      uniqueRoutes.map((routeIdSeq) => {
        const [routeId, seq] = routeIdSeq.split("/");
        return !navigator.onLine
          ? new Promise((resolve) => resolve([{ eta: 1 }]))
          : fetchEtas({
              ...routeList[routeId],
              seq: parseInt(seq, 10),
  ...
  return (
    <Paper className={"search-root"} square elevation={0}>
      ...
      <Box
        className={
          !energyMode ? "search-result-list" : "search-result-list-energy"
        }
      >
        {!locations.end ? (
          <RouteSearchDetails />
        ) : "waiting|rendering".includes(status) && result.length === 0 ? (
          <CircularProgress size={30} className={"search-route-loading"} />
